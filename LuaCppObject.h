...
public:
    ...
    template <class T> static void Register(lua_State* L)
        // -------------------------------------------------------
        // Registers the constructor for a given class as a global
        // -------------------------------------------------------
    {
        lua_pushcfunction(L, T::new_lua_obj);
        lua_setglobal(L, demangle(typeid(T).name()).c_str());
    }
    ...
    template <class T> static void Register(lua_State* L, int pos, bool)
        // ---------------------------------------------------------------------------
        // Registers the constructor for a given class in the table at position `pos`.
        // ---------------------------------------------------------------------------
    {
    ...
    template <class T> static T* Register(lua_State* L, bool)
        // ---------------------------------------------------------------------------
        // Registers the constructor for a given class in the table at position `pos`.
        // ---------------------------------------------------------------------------
    {
        ...
        lua_getglobal(L, demangle(typeid(T).name()).c_str());
        return (T*)lua_touserdata(L, 2);
    }
...
protected:
    ...
    static std::string demangle(const char* mname)
    {
        std::string demangledName = std::string(mname).substr(6);
...
protected:
        ...
        // ---------------------------------------------------------------------------
    {
        ...
        printf("killing %s with refid %d...\n", object->get_type().c_str(), object->__refid);
